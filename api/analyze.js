const SYSTEM_PROMPT_DESCRIBE = `你是一位客观的视觉描述者。你的任务是用平实的语言描述一幅"房树人"绘画的画面内容。
...
const SYSTEM_PROMPT_GUIDE = `## 知识库：房树人心理测验（HTP）分析理论框架
...
async function callVisionAPI(systemPrompt, userMessage, maxRetries = 2) {
  const apiKey = process.env.DASHSCOPE_API_KEY;
  if (!apiKey) {
    throw new Error("环境变量 DASHSCOPE_API_KEY 未设置");
  }
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`[ERROR] 通义千问 API 请求失败 (attempt ${attempt})`);
        console.error(`  响应状态码: ${response.status}`);
        console.error(`  响应内容: ${text.slice(0, 500)}`);
      }

      if (response.status === 401) {
        lastError = "API Key 无效或未授权";
        continue;
      }
      if (response.status === 429) {
        lastError = "请求频率过高，请稍后重试";
        continue;
      }
      if (!response.ok) {
        lastError = `通义千问 API 返回错误 (HTTP ${response.status})`;
        continue;
      }
      ...
      if (!content) {
        lastError = "解析 API 响应时出错: 缺少 content 字段";
        break;
      }
      return content.trim();
    } catch (e) {
  ...
  throw new Error(lastError || "AI 分析服务暂时不可用");
}
...
export default async function handler(req, res) {
  ...
  if (!image) {
    return res.status(400).json({ success: false, analysis: "请提供图片数据" });
  }

  if (typeof image !== "string") {
    return res.status(400).json({ success: false, analysis: "图片数据格式不正确" });
  }
  ...
  try {
    console.log("[INFO] 开始客观描述阶段");
    const description = await callVisionAPI(SYSTEM_PROMPT_DESCRIBE, userMessage);

    console.log("[INFO] 开始引导提问阶段");
    const guidePrompt = `以下是一幅房树人画作的客观描述，请根据此描述进行引导式提问：\n\n${description}`;
    const guideText = await callVisionAPI(SYSTEM_PROMPT_GUIDE, [
      { type: "text", text: guidePrompt },
    ]);

    return res.status(200).json({ success: true, analysis: guideText });
  } catch (e) {
    console.error(`[ERROR] 分析失败: ${e.message}`);
    if (e.message.includes("API Key") || e.message.includes("未设置")) {
      return res.status(500).json({ success: false, analysis: "服务配置错误，请联系管理员" });
    }
    return res.status(502).json({ success: false, analysis: e.message });
  }
}
